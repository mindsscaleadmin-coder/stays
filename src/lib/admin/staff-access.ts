...
export const ADMIN_ROUTE_PERMISSIONS: { prefix: string; permission: StaffPermission | null }[] = [
  ...
  { prefix: "/admin/subscription", permission: "manage_financial" },
  { prefix: "/admin/trust", permission: "manage_trust" },
  { prefix: "/admin/content", permission: "manage_content" },
  { prefix: "/admin/blog", permission: "manage_content" },
  { prefix: "/admin/advertisements", permission: "manage_settings" },
 ...
 */
const resolvedStaffByEmail = new Map<string, StaffMember>();
...
export function resolveStaffForAdminEmail(email: string): StaffMember | null {
  ...
  if (existing) {
    if (!existing.active) {
      resolvedStaffByEmail.delete(normalized);
      return null;
    }
    ...
    const cached = resolvedStaffByEmail.get(normalized);
    if (
      cached &&
      cached.id === next.id &&
    ...
    resolvedStaffByEmail.set(normalized, next);
    return next;
  }

  const cached = resolvedStaffByEmail.get(normalized);
  if (cached?.id === `virtual-${normalized}`) return cached;

  const virtual: StaffMember = {
  ...
  resolvedStaffByEmail.set(normalized, virtual);
  return virtual;
}
